# -*- coding: utf-8 -*-
#------------------------------------------------------------
# Multilink / Multiparser en PLD.VisionTV (juarrox@gmail.com)
...
def multiparser(params):
    ...
    encuentra = '#EXTINF:-1,' + params.get("title").replace(" [COLOR lightyellow][Multiparser][/COLOR]","").replace("[COLOR white]", "")
    encuentra=encuentra.split("[")[0];encuentra=encuentra.strip()  # Control para poder añadir metadatos al título en los filtros de películas
    if plugintools.get_setting("fpa_on"):
        title = '[COLOR white]'+title+'[/COLOR]'  # Control para lista única de pelis (LUPE)
        title = params.get("title");title=parser_title(title);title=title.split("[")[0].strip()
        encuentra = '#EXTINF:-1,'+title
            ...
            if data.startswith(encuentra) == True:
                ...
                if data == "#multiparser":
                    ...
                    while linea_url != "#multiparser" :                         
                        ...
                        if len(linea_url) == 3:
                            url_option = linea_url[1]+','+linea_url[2]
                        else:
            ...
            if data.startswith(encuentra) == True or data.startswith('#EXTINF:-1,@' + title.replace(" [COLOR lightyellow][Multilink][/COLOR]","")) == True:
                ...
                if data == "#multilink":
                    ...
                    while linea_url != "#multilink" :
                        linea_url = linea_url.strip().split(",")
                        # Parche para enlaces con comas (Stalker, Livestreamspro, etc)
                        subcomas = len(linea_url)
                ...
                try:
                    ...
                    print selector
                    if selector >= 0: url_analyzer(url_options[selector])
                    else: sys.exit()
