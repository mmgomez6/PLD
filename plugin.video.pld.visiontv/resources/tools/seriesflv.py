# -*- coding: utf-8 -*-
#------------------------------------------------------------
# Regex de SeriesFLV para PLD.VisionTV
# Version 0.1
#------------------------------------------------------------
...
def categorias_flv(data,show):
    plugintools.log('[%s %s] SeriesFLV regex %s' % (addonName, addonVersion))	
    plugintools.modo_vista(show)
...
def lista_chapters(params):
    plugintools.log('[%s %s] SeriesFLV Lista de capítulos: %s' % (addonName, addonVersion, repr(params)))
...
def chapter_urls(params):
    ...
    plugintools.add_item(action="", title='[COLOR gold][B]'+title+'[/COLOR]' , url = "" , info_labels = datamovie , extra = show , page = show , thumbnail = thumbnail , fanart = fanart , folder = False, isPlayable = False)
        ...
        if len(server_name) >= 2:
            ...
            if url == "": pass
                ...
                if url.find("allmyvideos") >= 0:
                    ...
                    plugintools.add_item(action="powvideo", title= '[COLOR white]'+title_fixed+' [/COLOR][COLOR lightyellow][I]('+server_name+')[/I][/COLOR]  [COLOR lightgreen][I]'+lang+'[/I][/COLOR]' , url = url, info_labels = datamovie , extra = show , page = show , fanart = fanart , thumbnail = thumbnail , folder = False, isPlayable = True)					
...
def lista_capis(params):
    plugintools.log("[%s %s] SeriesFLV Lista de capítulos" % (addonName, addonVersion))
