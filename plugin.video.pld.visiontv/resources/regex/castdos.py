...
def castdos(params):
    plugintools.log("[PLD.VisionTV-0.3.0].cast247 "+repr(params))
    url_user = {}
    ...
    if url_user.get("pageurl").find("domain=") <= 0:
        ...
        if body.find("Private Channel!") > 0 :
            xbmc.executebuiltin("Notification(%s,%s,%i,%s)" % ('PLD.VisionTV', "Canal privado", 3 , art+'icon.png'))
            return 0
        else:
