...
def LOGIN(username,password,hidesuccess):
	...
	if logged_in == True:
		...
		Bienve1 = dialogo.ok('[COLOR red]Bienvenidos a [/COLOR]***[COLOR blue]TVGUIA PROYECTOLUZDIGITAL[/COLOR]***','[COLOR red]     El plugin TVGUIA [COLOR blue]Proyectoluzdigital[/COLOR][COLOR red] esta Activado[/COLOR]   [/COLOR]','Esperamos Disfruteis de este plugin hecho para ustedes, PLD no se hace RESPONSABLE del uso que les den a este plugin esto es solo, con fines de investigacion.','        Visiten el foro para cualquier duda y consulta Gracias.    [COLOR blue]----------------------[COLOR blue] http://proyectoluzdigital.info[/COLOR]-----------------------[/COLOR] ')
		import activador
	...
	if logged_in == False:
		Notificaciones('[COLOR blue]Visite ProyectoLuzDigital.info[/COLOR]','El Usuario y Contraseña no son Correcto Introduzcalo de nuevo','5000',notierror)
	        ...
	        Bienve1 = dialogo.ok('Bienvenidos a [COLOR blue]PROYECTOLUZDIGITAL[/COLOR]','###### [COLOR red]El Usuario y Contraseña no son Correcto[/COLOR]######','########         Vuelva a intentarlo de nuevo       ########','###### O Visite  [COLOR blue] http://proyectoluzdigital.info [/COLOR] #######')
